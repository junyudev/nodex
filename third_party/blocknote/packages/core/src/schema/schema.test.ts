...
describe("CustomBlockNoteSchema.extend", () => {
  it("preserves child acceptance and layout rules while extending specs", () => {
    const schema = BlockNoteSchema.create({
      blockChildrenRules: {
        paragraph: { acceptance: { kind: "never" }, layout: "atomic" },
      },
    }).extend({ blockSpecs: { customBlock: createCustomBlockSpec() } });

    expect(schema.acceptsBlockChildren({ type: "paragraph" })).toBe(false);
    expect(schema.getBlockChildrenLayout("paragraph")).toBe("atomic");
    expect(schema.acceptsBlockChildren({ type: "customBlock" })).toBe(true);
  });
