...
describe("getBlocksChangedByTransaction", () => {
  ...
  it("should return blocks inserted by a transaction", async () => {
    const blocksChanged = editor.transact((tr) => {
      editor.insertBlocks([{ id: "0", type: "paragraph" }], "paragraph-0", "after");
      return getBlocksChangedByTransaction(tr);
    });
        ...
        [
          {
            id: "0",
            type: "paragraph",
            children: [{ id: "1", type: "paragraph", content: "Nested" }],
          },
        ],
        "paragraph-0",
        "after",
  ...
  it("should return multiple blocks, if multiple inserts add new blocks in a transaction", async () => {
    const blocksChanged = editor.transact((tr) => {
      editor.insertBlocks(
        [{ id: "0", type: "paragraph", content: "ABC" }],
        "paragraph-0",
        "after",
      );
      editor.insertBlocks(
        [{ id: "1", type: "paragraph", content: "DEF" }],
        "paragraph-1",
        "after",
      );
