...
- `Cmd/Ctrl+K` and `Cmd/Ctrl+P` open a global command palette that searches cards across projects by default; typing a `>` prefix switches the palette into command mode for shell actions such as opening settings, task search, project picker, terminal, stage focus, and view switches. Card results use fuzzy full-text ranking across title, description, tags, assignee, agent status, column, project name, and card id; matching terms are highlighted in titles and other matched metadata, and matching description results render a three-line contextual preview with inline highlights
