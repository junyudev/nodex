...
### Keep card->editor drops on Pragmatic Drag and Drop by bridging pointer state, not by adding a second drag runtime
Kanban card reorder now relies on Atlassian Pragmatic Drag and Drop's native element draggables and a board-level monitor. For card->editor imports, keep that same drag source and bridge with a lightweight global card-drag session (payload + live pointer) plus a registry of external editor drop targets resolved via `elementsFromPoint`. Do not introduce a second custom drag stack for editor imports; that recreates gesture skew and duplicated drop semantics.

### Pragmatic card drags still need a bridged editor insertion indicator
BlockNote/ProseMirror drop-cursor visuals are driven by editor-native drag/drop handling, not by Kanban's external card runtime. When card drags originate from the board, ProseMirror's built-in dropcursor is still not enough, so editor targets must render a synthetic insertion line from pointer-anchored block resolution (`elementsFromPoint` + block midpoint fallback). Keep indicator computation in the same helper used for actual insert placement so preview and final insertion cannot drift.
...
The board indicator expresses a visible slot in the rendered list, but `moveCard` / `moveCards` persist an insertion index after the dragged cards have been removed from the target column. Using the visible target index directly makes same-column drops land one position away from the indicator in forward moves. Use one shared visible-slot-to-persisted-order mapper for all drags, not only filtered/sorted boards, so the indicator and final result stay aligned. Keep that `newOrder` contract identical across the renderer optimistic transforms and `db-service`; if one side interprets it as a pre-removal/full-list index while the other uses post-removal order, same-column reorder regressions come back immediately.

### Sorted Kanban drag must disable card targets without disabling column targets
Under non-default Kanban sort, same-column manual ranking is intentionally disabled, but cross-column drag should still work. If one shared `dropDisabled` flag is wired into both card drop-target registration and column drop-target registration, sorted drags become visibly draggable but have nowhere valid to land on the board. Keep these layers separate: disable card targets in sorted mode, keep column targets active, and let the board-level drop strategy decide whether a resolved column hit becomes `move-only` or `none`.
...
### Kanban card drags and editor block imports must not co-own the same drop indicator state
Nodex has two native drag paths on the board surface: Pragmatic Drag and Drop for board card drags, and plain DOM `dragover` / `drop` handlers for editor block imports. If the editor-import handlers stay attached during a board card drag, they still receive native `dragover` events and can clear or overwrite the board indicator even though no editor drag session exists. That shows up as rapid indicator blinking. The fix is to make indicator ownership explicit: while a Kanban card drag is active, the native editor-import handlers must no-op completely and let the board monitor be the only writer of indicator state.
