...
The bundled Agent runtime is an immutable, checksum-verified upstream app-server package. Nodex does
not change or strengthen its internal mailbox and completion-delivery semantics, and therefore does
not claim crash-durable or exactly-once parent delivery. Runtime notifications are observations, not
the sole visible-state authority: Core retains the bounded descendant/status projection, Main
reconciles it after reconnect or app-server replacement, and renderers reread that projection after
root-scoped invalidation. Current-generation active metadata fences terminal evidence inherited from
an older generation, while `notLoaded` preserves uncertainty and cannot fabricate completion. This
keeps stale `active` rows from surviving reconnect and keeps stale completion delivery from hiding a
child that the current runtime reports active, without coupling Nodex releases to a private runtime
fork.
