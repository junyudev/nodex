...
| Subagent observed descendant identity, status evidence and discovery facts                 | Rust Core Workspace Module                                                   | Main `CodexSubagentDirectory` reconciles observations; shared pure rows drive all presentations; app-server owns the execution graph and lifecycle |
...
[`crates/nodex-cli`](crates/nodex-cli) is a native Adapter over the same Core contracts used by Desktop. It resolves explicit Profile and Project scope, performs bounded reads and semantic writes, and never accesses private Store SQL or private lifecycle authority. It does not depend on Electron and does not bypass Module authorization. The one offline provisioning exception is `profile clone`: because its target Core does not exist yet, the CLI invokes the Profile provisioning Module in-process. That outer Module composes an unpublished Core Administration Store clone with the backend-owned conversation closure, verifies local Thread requirements across those owners, and publishes one new Profile directory. Core reads only a published evidence-backed Store backup, preserves imported Store lineage, and remints instance secrets; it never interprets native transcripts. The native Adapter owns capture consistency and relocation of its selected rollout paths. Ordinary reads and mutations remain protocol-only. The result is an isolated local fork whose post-clone history is never merged or replayed into its source Profile. See [Profile clone reliability](reliability/backup-restore-and-maintenance.md#development-profile-clones) for capture boundaries and failure behavior.
...
Subagent discovery and control belong to Main's root-scoped `CodexSubagentDirectory`.
The app-server owns the execution graph, transcripts and native descendant lifecycle operations.
Core stores observed identity and status evidence as a restart-safe index. One pure shared row
projection derives presentation from those facts and resident canonical history; overview,
transcript, summary and mentions do not maintain independent status or messaging policies.
Host generations and durable Project/host coordinates fence discovery, selection and control.

Parent surfaces consume metadata and pending-request projections without attaching sibling
transcripts. Selecting one child delegates history loading to the existing history and peer
Modules. Read-only attachment can fetch history without native execution resume or writer
acquisition. Messaging authority comes from the immediate parent's canonical history and remains
separate from status and openability. Native archive/delete notifications drive local cleanup;
client-side graph completeness and lifecycle closures do not gate those operations.
...
The Profile Scope owns one `DictationRuntime` for microphone/settings/history/global-routing resources and one `CodexMedia` capability for account-scoped policy, authenticated transcription, and streaming preparation. Account dictionary requests share that policy and retain their original account identity. `CodexMedia` derives and publishes the single capability snapshot from account, connection, streaming, and native-helper changes; a scoped `DictationIpc` Layer is the only renderer transport. Composer detachment transfers pending text reconciliation to its editor owner and persisted draft while activation remains exclusive until settlement. Explicit cancellation, helper/window teardown, and IPC removal follow the Main application Scope instead of an independent shutdown graph.

The Host Scope owns authenticated workspace routing independently of media. The routing Module can use the verified bundled runtime when the primary runtime lacks supported discovery, while retaining the primary account, host generation, and network/residency authority. Companion credentials and process resources remain confined to a short-lived Scope; renderers never receive its RPC or filesystem capability.

Platform adapters combine operating-system shortcut registration with modifier-release observation and clipboard-safe foreground paste. macOS also uses a narrowly scoped signed helper for bare modifiers and microphone routing; Windows supplies its own native boundary. Binding replacement is atomic and generation-stamped. The Profile Scope owns the helper/window resources and a single supervised recovery loop. Main gives the focused Nodex window a bounded, request-correlated handoff; that renderer's sole in-app router admits only a visible focus-owning Composer target before falling back to a separate non-activating BrowserWindow with a dictation-only preload. That auxiliary renderer cannot access Core, filesystem, Workbench, or general application IPC.
