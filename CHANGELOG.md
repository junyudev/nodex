...
- Local Full access Agent tasks can now work with Nodex through the bundled CLI, with automatic task context, compact command discovery, direct text streams, bare resource IDs, SQL reads across Page bodies, properties, saved Views and Files, ranked Page search, atomic schema/View configuration, Property batches that preserve the original read versions, and atomic Page creation batches.
...
- Library Files now exist independently of Pages, can be browsed, shared, versioned, granted, copied, and cleaned up from one Library surface, while each Page keeps only optional paths and body references. Block move/copy and keyboard cut/paste preserve shared File identity, Page and Canvas history read exact captured versions, and opt-in local-path copy materializes the authorized version for other desktop apps. File details preserve selected identities and versions, show navigable usage locations and deletion constraints, and distinguish Page paths from shared content updates.
