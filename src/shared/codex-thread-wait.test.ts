...
function fixture() {
  ...
  const manager: ThreadWaitManager = {
    ...
    read: async (id) => {
      ...
      return { status: status.get(id) ?? { type: "active", activeFlags: [] }, latestTurn: turn };
    },
...
test("completion wakes group, reports earlier errors and releases other subscriptions", async () => {
  ...
  const waiting = waitForThreads(
    client,
    ["missing", "a", "b"].map((threadId) => ({ threadId, hostId: "local" })),
    120000,
  ...
  expect(result.errors).toEqual([
    { threadId: "missing", hostId: "local", message: "missing thread" },
  ]);
...
test("unchanged completed target returns without wake and does not repeat text", async () => {
  ...
  status.set("a", { type: "idle" });
  const target = { threadId: "a", hostId: "local" };
  const first = await waitForThreads(client, [target], 0, new AbortController().signal);
  const repeated = await waitForThreads(
    client,
    [{ ...target, afterCursor: first.polls[0]!.cursor }],
    120000,
    new AbortController().signal,
  );
  expect(repeated.wake).toBeNull();
  expect(repeated.timedOut).toBe(false);
  expect(repeated.polls[0]?.changed).toBe(false);
});
