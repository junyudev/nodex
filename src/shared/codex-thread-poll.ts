...
export interface ThreadPollTarget {
...
interface AssistantMessage {
  ...
  readonly phase: Extract<ThreadItem, { type: "agentMessage" }>["phase"];
...
interface Projection {
  ...
  readonly latestTurn:
    | (Pick<Turn, "id" | "status" | "startedAt" | "completedAt" | "durationMs"> & {
...
export interface ThreadPoll extends Projection {
  ...
  readonly cursorReset?: true;
  readonly latestAssistantMessageId: string | null;
  readonly latestToolMarkerId: string | null;
}
export type ThreadPollWake =
  | { readonly reason: "turnCompleted"; readonly turnId: string }
  | { readonly reason: "inactiveStatus" | "actionableStatus" };
...
export function isNewerPollTurn(
  a: NonNullable<Projection["latestTurn"]>,
  b: NonNullable<Projection["latestTurn"]>,
): boolean {
...
function assistant(turn: Turn | null): AssistantMessage | null {
  ...
  const lastWork =
    turn.status === "inProgress"
      ? turn.items.findLastIndex(
          (item) => item.type !== "userMessage" && item.type !== "hookPrompt",
        )
      : -1;
  for (let index = turn.items.length - 1; index >= 0; index -= 1) {
    ...
    const text = projectCodexMarkdownToPlainText(
      heartbeat ? heartbeat.visibleText || heartbeat.notificationMessage || "" : content,
    );
    if (!text) continue;
    return {
      id: item.id,
      ...
      ...(text.length > 2000 ? { truncated: true, originalChars: text.length } : {}),
    };
  }
  return null;
}
function tool(turn: Turn | null): ToolMarker | null {
  ...
  for (let index = turn.items.length - 1; index >= 0; index -= 1) {
    ...
    switch (item.type) {
      ...
      case "sleep":
      case "webSearch":
        return { id: item.id, turnId: turn.id, type: item.type, name: item.type, status: null };
    }
  }
  return null;
}
...
export class ThreadPollCache {
  ...
  read(target: ThreadPollTarget): Projection | undefined {
    return this.entries.get(JSON.stringify([target.hostId, target.threadId]))?.projection;
  }
  unchangedStatus(target: ThreadPollTarget): ThreadStatus | null {
    ...
    return entry && cursorRevision(target.afterCursor, entry.generation) === entry.revision
      ? entry.projection.thread.status
      : null;
  }
  commit(
    ...
    return this.commitProjection(
      target,
      {
        thread: { id: target.threadId, hostId: target.hostId, status },
        latestTurn: turn
          ? {
              id: turn.id,
              status: turn.status,
              error: turn.error ? { message: turn.error.message.slice(0, 2000) } : null,
        ...
        latestAssistantMessage:
          assistant(turn) ??
          (preserveMissingItems && turn && prior?.latestTurn?.id === turn.id
            ? prior.latestAssistantMessage
            : null),
        latestToolMarker:
          tool(turn) ??
          (preserveMissingItems && turn && prior?.latestTurn?.id === turn.id
            ? prior.latestToolMarker
            : null),
      },
      ordinal,
    );
  ...
  fallback(
    ...
    if (completed)
      return this.commit(
        target,
        status?.type === "active" && status.activeFlags.length > 0 ? status : { type: "idle" },
        completed,
        this.beginRead(),
        true,
      );
    return this.commitProjection(
      target,
      {
        ...
        thread: {
          ...
          status: status ?? prior!.thread.status,
        },
      },
      this.beginRead(),
    );
  ...
  withProgress(poll: ThreadPoll): ThreadPoll {
    ...
    return {
      ...poll,
      latestAssistantMessage: current?.latestAssistantMessage ?? poll.latestAssistantMessage,
      latestToolMarker: current?.latestToolMarker ?? poll.latestToolMarker,
    };
  }
  private commitProjection(
    ...
    if (
      ...
      priorTurn &&
      (!turn ||
        isNewerPollTurn(priorTurn, turn) ||
        (priorTurn.id === turn.id &&
          priorTurn.status !== "inProgress" &&
          turn.status === "inProgress"))
    )
      projection = { ...previous.projection, thread: candidate.thread };
    else if (
      turn?.status === "inProgress" &&
      oldMessage &&
      message &&
      oldMessage.id === message.id &&
      oldMessage.text.length > message.text.length
    )
      projection = { ...candidate, latestAssistantMessage: oldMessage };
    const fingerprint = JSON.stringify({
      status: projection.thread.status,
      turnId: projection.latestTurn?.id ?? null,
      turnStatus: projection.latestTurn?.status ?? null,
      turnError: projection.latestTurn?.error?.message ?? null,
      assistant: projection.latestAssistantMessage,
      tool: projection.latestToolMarker,
    });
    const entry = {
  ...
  private project(entry: CacheEntry, cursor: string | undefined): ThreadPoll {
    ...
    return {
      ...entry.projection,
      schemaVersion: 1,
      cursor: `${entry.generation}:${entry.revision}`,
      revision: entry.revision,
      changed,
      ...(reset ? { cursorReset: true } : {}),
      latestAssistantMessageId: entry.projection.latestAssistantMessage?.id ?? null,
      latestToolMarkerId: entry.projection.latestToolMarker?.id ?? null,
      latestAssistantMessage: changed ? entry.projection.latestAssistantMessage : null,
      latestToolMarker: changed ? entry.projection.latestToolMarker : null,
    };
  }
}
export function statusPollWake(status: ThreadStatus): ThreadPollWake | null {
  if (status.type === "systemError" || status.type === "notLoaded")
    return { reason: "inactiveStatus" };
  if (status.type === "active" && status.activeFlags.length) return { reason: "actionableStatus" };
  return null;
}
export function unchangedTerminalPoll(poll: ThreadPoll): boolean {
  return (
    !poll.changed &&
    poll.thread.status.type === "idle" &&
    poll.latestTurn !== null &&
...
export function pollWake(poll: ThreadPoll): ThreadPollWake | null {
  ...
  return poll.latestTurn.status === "inProgress" || unchangedTerminalPoll(poll)
    ? null
    : { reason: "turnCompleted", turnId: poll.latestTurn.id };
}
