...
function searchPositions(
  ...
  lookup: (key: string) => CodexCanonicalTurnState | undefined,
  ...
  const anchorIndex = entries.findIndex((entry) => {
    const id = lookup(entry.value)?.turnId;
    return id != null && identities.has(id);
  });
  if (anchorIndex !== -1) {
...
export function insertCanonicalHistorySearchDraft(
  ...
  if (history.islands.some((island) => island.id === id))
    throw new Error(`Duplicate history island ID: ${id}`);
  ...
  const boundary = (cursor: string | null, edge: "older" | "newer") =>
    cursor == null
      ? exhaustedCodexHistoryBoundary(`${id}:${edge}`)
      : availableCodexHistoryBoundary(
          `${id}:${edge}`,
          { cursor, oldestLoadedTurnId: input.turns[0]?.turnId ?? null },
          cursor,
        );
  const dedupe = (entries: readonly CodexHistoryEntry[]) => {
    ...
    for (const entry of entries) {
      ...
      if (previous !== undefined) {
        if (previous !== entry.value) retained.delete(entry.value);
        continue;
      }
  ...
  const island: CodexHistoryIsland = {
    id,
    entries: dedupe(incoming),
    olderBoundary: boundary(input.olderCursor, "older"),
    newerBoundary: boundary(input.newerCursor, "newer"),
  };
  const range = (value: CodexHistoryIsland) => {
    ...
    for (const entry of value.entries) {
      ...
      if (position === undefined)
        throw new Error(`Missing search history position for ${entry.value}`);
  ...
  const overlaps = history.islands.filter((value) => {
    const current = range(value);
    return (
      current !== null && current.first <= incomingRange.last && incomingRange.first <= current.last
    );
  });
  ...
  const merged = {
    id: last.id,
    entries: dedupe([...overlaps.flatMap((value) => value.entries), ...island.entries]),
    olderBoundary: first.olderBoundary,
    newerBoundary: last.newerBoundary,
  };
  const islands = [...history.islands.filter((value) => !overlaps.includes(value)), merged].sort(
    (a, b) => {
      const left = range(a);
      const right = range(b);
      return left === null || right === null ? 0 : left.first - right.first;
    },
  );
  for (const [key, turn] of Object.entries(installed))
    if (retained.has(key)) history.entitiesByKey[key] = turn as Draft<CodexCanonicalTurnState>;
  history.islands = islands as Draft<CodexHistoryIsland[]>;
  const entries = residentConversationTurnEntries(state);
  const turns = relocateCodexHydratedSteering(
    residentConversationTurns(state),
    (turnId) =>
      residentConversationTurns(state).find((turn) => turn.turnId === turnId)?.itemsPagination,
  );
  turns.forEach((turn, index) => {
    const current = conversationTurnDraft(state, entries[index]!.address);
    if (current && current.items !== turn.items)
      current.items = turn.items as Draft<CodexCanonicalTurnState["items"]>;
  });
  history.isComplete =
    islands.length === 1 &&
...
export async function hydrateCanonicalHistorySearchMatch(
  client: CanonicalHistoryClient,
  input: CanonicalHistorySearchMatch,
  createSearchIslandId: () => string,
): Promise<void> {
  const options = { priority: "interactive", source: "thread_hydration" } as const;
  const [older, newer] = await Promise.all(
    ["desc", "asc"].map((sortDirection) =>
      listCanonicalHistoryTurns(client, input.conversationId, {
  ...
  if (!client.getConversation(input.conversationId))
    throw new Error("Conversation disappeared while hydrating a search match");
  ...
  while (!seen.has(input.itemId)) {
    ...
    const page = await readCanonicalHistoryItems(
      ...
      input.turnId,
      progress.olderCursor,
      options,
    );
    for (const item of page.items.filter((item) => !seen.has(item.id)).reverse()) {
      seen.add(item.id);
      items.push(item);
    }
    match.itemsView = page.nextCursor == null ? "full" : "summary";
    progress.olderCursor = page.nextCursor;
    progress.isLoadingOlder = false;
    progress.hasLoadedOldest = page.nextCursor == null;
    pagination[input.turnId] = progress;
  }
  if (!match || !seen.has(input.itemId))
    throw new Error("Persisted conversation search match is no longer available");
  ...
  let applied = false;
  client.updateConversation(input.conversationId, (draft) => {
    applied = insertCanonicalHistorySearchDraft(draft, createSearchIslandId, {
      turns: mapped,
      olderCursor: older!.response.nextCursor,
      newerCursor: newer!.response.nextCursor,
    });
  });
