...
export interface CanonicalEditClient {
  ...
  readonly readPermissionOverrides: (
    id: string,
  ...
  readonly applyRevert: (
    id: string,
    response: ThreadRevertResponse,
    turns: readonly CodexCanonicalTurnState[],
  ) => void;
  readonly applyRollback: (
    id: string,
  ...
  readonly start: (
    ...
    original: CodexCanonicalTurnState,
    inheritPermissionDefaults: boolean | undefined,
  ) => Promise<unknown>;
}
...
export function replaceCanonicalEditedPrompt(original: string, message: string): string {
  ...
  return marker === undefined
    ? message
    : `${original.slice(0, marker.index).trimEnd()}\n## My request:\n${message}\n`;
}

/** Runs on the owner after follower dispatch; preserves the original message's non-text inputs. */
export async function editCanonicalLastUserTurn(
  client: CanonicalEditClient,
  id: string,
  options: CanonicalEditOptions,
): Promise<void> {
  ...
  const original =
    initial &&
    mergeCodexCanonicalTurnStates(residentConversationTurns(initial), initial.turns).find(
      (turn) => turn.turnId === options.turnId,
    );
  ...
  const input = original.params.input.map((item, index) =>
    item.type === "text" && index === textIndex
      ? {
          ...item,
          text: replaceCanonicalEditedPrompt(item.text, options.message),
          text_elements: [],
        }
      : item,
  );
  const useRevert = state.historyMode === "paginated" && client.supportsRevert();
  if (!useRevert && state.historyMode === "paginated")
    throw new Error("Editing messages is not available for threads using paginated history yet.");
  ...
  if (
    ...
    index === -1 ||
    removed.slice(1).some((turn) => turn.params.input.length > 0)
  )
    throw new Error("Only the most recent message can be edited.");
  if (removed.some((turn) => turn.status === "inProgress"))
    throw new Error("Cannot edit a message while a turn is in progress.");
  const permissionFactory = options.shouldSendPermissionOverrides
    ? await client.readPermissionOverrides(id, state, options)
    : null;
  let thread: Thread;
  if (useRevert) {
    ...
    const response = await client.rollback(
      id,
      removed.filter((turn) => turn.turnId !== null).length,
    );
    thread = response.thread;
  ...
  await client.start(
    ...
    original,
    permissions == null ? true : undefined,
  );
}
