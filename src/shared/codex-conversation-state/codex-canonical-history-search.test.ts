...
function fixture() {
  ...
  const turn = (turnId: string, turnStartedAtMs: number | null) => ({ ...template, turnId, turnStartedAtMs, items: [] });
  return { state, turn };
}
...
describe("persisted search history placement", () => {
  test("overlap preserves resident entry identity and places surrounding turns without timestamps", () => {
    ...
    let applied = false;
    const after = produce(before, (draft) => { applied = insertCanonicalHistorySearchDraft(draft, () => "window", { turns: [f.turn("older", null), f.turn("anchor", null), f.turn("newer", null)], olderCursor: "older-page", newerCursor: "newer-page" }); });
    expect(applied).toBe(true);
    expect(residentConversationTurns(after).map((turn) => turn.turnId)).toEqual(["older", "anchor", "newer"]);
    expect(after.turnHistory!.history.islands[0]!.entries[1]).toEqual({ key: "visible-stable", value: "local-stable" });
  ...
  test("refuses disconnected windows when any resident timestamp is unknown", () => {
    const f = fixture();
    const before = produce(f.state, (draft) => replaceCanonicalHistoryDraft(draft, [f.turn("resident", null)], false));
    let applied = true;
    const after = produce(before, (draft) => { applied = insertCanonicalHistorySearchDraft(draft, () => "window", { turns: [f.turn("match", 10)], olderCursor: null, newerCursor: null }); });
  ...
  test("orders disconnected windows by timestamp with deterministic Turn identity ties", () => {
    const f = fixture();
    const before = produce(f.state, (draft) => replaceCanonicalHistoryDraft(draft, [f.turn("z", 20)], false));
    const after = produce(before, (draft) => { insertCanonicalHistorySearchDraft(draft, () => "window", { turns: [f.turn("a", 20)], olderCursor: "older", newerCursor: "newer" }); });
