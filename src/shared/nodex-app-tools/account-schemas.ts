...
const descriptions: Record<keyof typeof accountSchemas, string> = {
  get_usage_limits:
    "Read current Codex account usage windows and available reset credits on this host. Limits are shared across the account. ordinaryUsageAllowed is the backend authority for ordinary included usage; null means unavailable, so do not infer availability from percentages or reset times. usedPercent is consumed; remaining percent is 100 minus usedPercent, clamped to 0–100. Resets are Unix seconds. Prefer rateLimitsByLimitId when available; null means unavailable, never zero usage. Does not redeem or purchase anything.",
  consume_usage_reset:
