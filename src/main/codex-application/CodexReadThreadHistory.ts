...
export class CodexReadThreadHistoryError extends Data.TaggedError("CodexReadThreadHistoryError")<{
  readonly threadId: string;
  readonly reason: "unknown-cursor" | "request-failed";
...
const output = (text: string, limit: number) =>
  text.length <= limit
    ? { text, truncated: false }
    : { text: text.slice(0, limit), truncated: true, originalChars: text.length };
function userInput(input: UserInput): Record<string, unknown> {
  switch (input.type) {
    case "text": {
      const delegation = parseCodexDelegationText(input.text);
      return {
        type: input.type,
        text: input.text,
        ...(delegation ? { codexDelegation: delegation } : {}),
      };
    }
    case "image":
    case "audio":
    ...
    case "skill":
    case "mention":
      return { type: input.type, name: input.name, path: input.path };
  }
}
...
export function serializeCodexReadThreadProtocolItem(
  ...
  switch (item.type) {
    case "userMessage":
      return { ...identity, content: item.content.map(userInput) };
    case "agentMessage":
      return { ...identity, text: item.text, phase: item.phase };
    case "plan":
      return { ...identity, text: item.text };
    case "reasoning":
      return {
        ...identity,
        summary: item.summary,
        ...(includeOutputs ? { content: item.content.map((text) => output(text, limit)) } : {}),
      };
    case "commandExecution":
      return {
        ...
        ...(includeOutputs && item.aggregatedOutput !== null
          ? { output: output(item.aggregatedOutput, limit) }
          : {}),
      };
    case "fileChange":
      return {
        ...
        changes: item.changes.map((change) => ({
          path: change.path,
          kind: change.kind,
          ...(includeOutputs ? { diff: output(change.diff, limit) } : {}),
        })),
      };
    case "mcpToolCall":
      return {
    ...
    case "webSearch":
      return { ...identity, query: item.query, action: item.action };
    case "imageView":
      return { ...identity, path: item.path };
    case "sleep":
      return { ...identity, durationMs: item.durationMs };
    ...
    case "exitedReviewMode":
      return { ...identity, review: item.review };
    case "hookPrompt":
      return { ...identity, fragmentCount: item.fragments.length };
...
export const make = Effect.gen(function* () {
  ...
  return CodexReadThreadHistory.of({
    read: (input) =>
      Effect.gen(function* () {
        const limit = input.turnLimit ?? CODEX_APP_READ_THREAD_DEFAULT_TURN_LIMIT;
        const maxOutput =
          input.maxOutputCharsPerItem ?? CODEX_APP_READ_THREAD_DEFAULT_MAX_OUTPUT_CHARS;
        if (
          !Number.isInteger(limit) ||
          limit < 1 ||
          limit > CODEX_APP_READ_THREAD_MAX_TURN_LIMIT ||
          !Number.isInteger(maxOutput) ||
          maxOutput < 0 ||
          maxOutput > CODEX_APP_READ_THREAD_MAX_OUTPUT_CHARS
        )
          return yield* new CodexReadThreadHistoryError({
            threadId: input.threadId,
            reason: "request-failed",
            cause: new Error("read_thread received invalid arguments."),
          });
        const resolved = yield* lookup.resolve(input.threadId, input.hostId, { timeoutMs: 30000 });
        const { hostId, manager } = resolved;
        const assertCurrent = Effect.try({
          try: manager.assertCurrent,
          catch: (cause) =>
            new CodexReadThreadHistoryError({
              threadId: input.threadId,
              reason: "request-failed",
              cause,
            }),
        ...
        if (thread.historyMode === "paginated" || capability.flags.paginatedHistory) {
          ...
          const cursor =
            legacy && input.cursor !== null && input.cursor !== undefined
              ? JSON.stringify({ turnId: input.cursor, includeAnchor: false })
              : input.cursor;
          const page = yield* gateway.requestOnHost(
            hostId,
            "thread/turns/list",
            { threadId: input.threadId, cursor, itemsView: "full", limit },
            options,
          );
          ...
          thread = (yield* gateway.requestOnHost(
            hostId,
            "thread/read",
          ...
          const end =
            input.cursor === null || input.cursor === undefined
              ? thread.turns.length
              : thread.turns.findIndex((turn) => turn.id === input.cursor);
          if (end < 0)
            return yield* new CodexReadThreadHistoryError({
              threadId: input.threadId,
              reason: "unknown-cursor",
              cause: new Error(`Unknown cursor for thread ${input.threadId}: ${input.cursor}`),
            });
          const preceding = thread.turns.slice(0, end);
          turns = preceding.slice(-limit).reverse();
          hasMore = preceding.length > turns.length;
        ...
        return {
          ...
          page: { order: "newest_first", limit, nextCursor, hasMore },
          turns: turns.map((turn) => ({
            id: turn.id,
            status: turn.status,
            error: turn.error
              ? { message: turn.error.message, additionalDetails: turn.error.additionalDetails }
              : null,
            startedAt: turn.startedAt,
            completedAt: turn.completedAt,
            durationMs: turn.durationMs,
            items: turn.items.map((item) =>
              serializeCodexReadThreadProtocolItem(item, input.includeOutputs === true, maxOutput),
            ),
          })),
        } satisfies CodexReadThreadHistoryResult;
      }).pipe(
        Effect.mapError((cause) =>
          cause instanceof CodexReadThreadHistoryError
            ? cause
            : new CodexReadThreadHistoryError({
                threadId: input.threadId,
                reason: "request-failed",
