...
const input = {
  ...
  projectId: "project",
  prompt: "Hello",
} as CodexThreadStartForSessionInput;
const context = {
  ownerClientId: "window-peer",
  browserViewScopeId: "window",
  presentationClaim: { ticketId: "ticket", submissionId: "submission" },
};
const origin = {
...
const harness = () => {
  ...
  const runtime = make.pipe(
    Effect.provideService(
      CodexSessionThreadLaunch,
      CodexSessionThreadLaunch.of({
        start: (_input, scope: CodexSessionThreadLaunchContext) =>
          Effect.gen(function* () {
            events.push("prepared");
            const response = yield* scope.sendNativeStart!(
              { cwd: "/workspace" },
            ...
            return {
              kind: "started",
              detail: { threadId: response.thread.id },
...
it.effect("returns native response before durable acceptance under the caller identity", () =>
  Effect.gen(function* () {
    const h = harness();
    ...
    assert.strictEqual(
      yield* service.execute(prepared.receiptId, context.ownerClientId, origin),
      nativeResponse,
    );
    assert.deepEqual(h.events, ["prepared", "native"]);
    assert.isTrue(
      Exit.isFailure(
        yield* Effect.exit(service.execute(prepared.receiptId, context.ownerClientId, origin)),
      ),
    );
...
it.effect("window retirement cancels suspended acceptance and releases the origin claim", () =>
  Effect.gen(function* () {
    const h = harness();
    const service = yield* h.runtime;
    const prepared = yield* service.prepare(input, context);
