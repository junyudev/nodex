...
export const install = Effect.gen(function* () {
  ...
  yield* events.events.pipe(
    Stream.runForEach((event) => {
      ...
      return browser
        .endSessionActivity(event.value)
        .pipe(Effect.catch((cause) => Effect.logWarning("Browser activity release failed", cause)));
    }),
    ...
    Stream.runForEach((event) =>
      Effect.gen(function* () {
        ...
        if (
          ...
          parsed.data.notification.method !== "thread/status/changed" ||
          parsed.data.notification.params.status.type !== "idle"
        )
          return;
        ...
        Effect.catch((cause) => Effect.logWarning("Browser idle activity release failed", cause)),
      ),
    ),
    Effect.forkScoped({ startImmediately: true }),
  );
