...
import {
  ...
  type CodexRendererNativeRequestOrigin,
...
import {
  ...
  type CodexSessionThreadLaunchContext,
} from "./CodexSessionThreadLaunch";

export class CodexRendererSessionLaunch extends Context.Service<
  CodexRendererSessionLaunch,
  {
    readonly prepare: (
      input: CodexThreadStartForSessionInput,
      context: CodexSessionThreadLaunchContext,
    ) => Effect.Effect<CodexNativeSessionLaunchPreparation, CodexSessionThreadLaunchError>;
    readonly execute: (
      receiptId: string,
      ownerClientId: string,
      origin: CodexRendererNativeRequestOrigin,
    ) => Effect.Effect<ThreadStartResponse, CodexSessionThreadLaunchError>;
    readonly accept: (
      receiptId: string,
      ownerClientId: string,
...
interface Entry {
  ...
  readonly preparation: Deferred.Deferred<
    CodexNativeSessionLaunchPreparation,
    CodexSessionThreadLaunchError
  >;
...
export const make: Effect.Effect<
  CodexRendererSessionLaunch["Service"],
  never,
  CodexSessionThreadLaunch | CodexGateway | CodexTurnPresentation | Scope.Scope
> = Effect.gen(function* () {
  ...
  const failure = (sessionId: string, cause: unknown) =>
    new CodexSessionThreadLaunchError({ operation: "start", sessionId, cause });
  const lookup = (id: string, owner: string) =>
    Effect.gen(function* () {
      const entry = entries.get(id);
      if (!entry || entry.context.ownerClientId !== owner)
        return yield* Effect.fail(
          failure(entry?.input.sessionId ?? "", new Error("Session launch receipt unavailable")),
        );
      return entry;
    });
  ...
  return CodexRendererSessionLaunch.of({
    prepare: (input, context) =>
      Effect.gen(function* () {
        if (!context.ownerClientId || input.runInTarget === "newWorktree")
          return yield* Effect.fail(
            failure(
              input.sessionId,
              new Error("Native Session launch requires an immediate window target"),
            ),
          );
        ...
        const entry: Entry = {
          input,
          context,
          preparation: yield* Deferred.make<
            CodexNativeSessionLaunchPreparation,
            CodexSessionThreadLaunchError
          >(),
          response: yield* Deferred.make<ThreadStartResponse, CodexSessionThreadLaunchError>(),
          acceptance: yield* Deferred.make<void>(),
          dispatched: false,
        };
        entries.set(id, entry);
        const operation = launches
          .start(input, {
            ...context,
            sendNativeStart: (request, capability) =>
              Effect.gen(function* () {
                const prepared = {
                  receiptId: id,
                  request,
    ...
    execute: (id, owner, origin) =>
      Effect.gen(function* () {
        const entry = yield* lookup(id, owner);
        const prepared = entry.prepared;
        if (!prepared || entry.dispatched)
          return yield* Effect.fail(
            failure(
              entry.input.sessionId,
              new Error("Native Session launch was already dispatched"),
            ),
          );
        ...
        const response = yield* gateway
          .requestOnHost(
            prepared.hostId,
            "thread/start",
            prepared.request as ClientRequestParamsByMethod["thread/start"],
            codexGatewayGenerationFence(prepared),
          )
          .pipe(
    ...
    accept: (id, owner) =>
      Effect.gen(function* () {
        const entry = yield* lookup(id, owner);
        if (!entry.dispatched || !entry.fiber)
          return yield* Effect.fail(
            failure(entry.input.sessionId, new Error("Native Session launch has not been sent")),
          );
        yield* Deferred.await(entry.response);
        yield* Deferred.succeed(entry.acceptance, undefined);
        return yield* Fiber.join(entry.fiber).pipe(
          Effect.onExit((exit) =>
            Exit.isSuccess(exit)
              ? Effect.sync(() => {
                  entries.delete(id);
