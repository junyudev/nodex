...
export const make: Effect.Effect<
  CodexConversationProjectionService,
  never,
  ConversationEntityMap | CodexApplicationEventHub | CoreModules
> = Effect.gen(function* () {
  ...
  const commitStatus = (
    ...
    Effect.gen(function* () {
      aggregate(threadId)?.setThreadStatus(statusType);
  ...
  return CodexConversationProjection.of({
      ...
      Effect.try({
        try: () => {
          ...
          conversation.initializeHistory(
            input.pagination,
            residentConversationTurns(input.canonical).length,
    ...
    acceptTurn: (input) =>
      requireChanged("accept-turn", input.threadId, (conversation) =>
        conversation.acceptOptimisticTurn({
          permissions: input.permissions,
          execution: input.execution,
          environmentSelectionEvidence: input.environmentSelectionEvidence,
    ...
    commitInterruptedTurn: (input) =>
      Effect.gen(function* () {
        ...
        const turn = residentConversationTurns(canonical).find(
