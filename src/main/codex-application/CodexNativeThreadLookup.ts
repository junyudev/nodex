...
export class CodexNativeThreadLookup extends Context.Service<CodexNativeThreadLookup, {
  readonly resolve: (threadId: string, preferredHostId?: string, options?: CodexGatewayRequestOptions) => Effect.Effect<NativeThreadMatch, CodexNativeThreadLookupError>;
...
export const make = Effect.gen(function* () {
  ...
  return CodexNativeThreadLookup.of({ resolve: (threadId, preferredHostId, options) => Effect.gen(function* () {
    const hostIds = [gateway.localHostId, ...(yield* SubscriptionRef.get(hosts.activeSshHosts)).keys()];
    const probe = (hostId: string) => Effect.gen(function* () {
      const manager = yield* managers.get(hostId);
      const response = yield* gateway.requestOnHost(hostId, "thread/read", { threadId, includeTurns: false }, { ...options, expectedHostId: hostId, expectedGeneration: manager.generation });
      yield* Effect.try({ try: manager.assertCurrent, catch: (cause) => new CodexNativeThreadLookupError({ threadId, cause }) });
    ...
    const results = yield* Effect.forEach(hostIds.filter((hostId) => !failedHosts.includes(hostId)), (hostId) => probe(hostId).pipe(Effect.map((match) => ({ hostId, match }))), { concurrency: "unbounded" });
    failedHosts.push(...results.filter((result) => result.match === null).map((result) => result.hostId));
