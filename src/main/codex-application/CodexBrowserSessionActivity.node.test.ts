...
it.effect(
  "interruption and current native idle release browser activity independently of turn completion",
  () =>
    Effect.gen(function* () {
      ...
      yield* install.pipe(
        ...
        Effect.provideService(BrowserUseRuntime, {
          endSessionActivity: (id: string) =>
            Effect.sync(() => {
              calls.push(id);
            }).pipe(
              Effect.andThen(
                Deferred.succeed(id === "interrupted" ? interrupted : idle, undefined),
              ),
            ),
        } as unknown as BrowserUseRuntime["Service"]),
        Effect.provideService(CodexGateway, {
          events: Stream.fromPubSub(native),
          connection: () => Effect.succeed({ kind: "ready", hostId: "local", generation: 2 }),
        } as unknown as CodexGateway["Service"]),
      );
      events.publish({ kind: "conversationTurnInterruptStarted", value: "interrupted" });
      yield* Deferred.await(interrupted);
      yield* PubSub.publish(native, {
        kind: "notification",
        hostId: "local",
        generation: 1,
        value: {
          ...
          params: { threadId: "stale", status: { type: "idle" } },
        },
      });
      yield* PubSub.publish(native, {
        kind: "notification",
        ...
        value: {
          protocol: "extension",
          method: "thread/status/changed",
          params: { threadId: "idle", status: { type: "idle" } },
        },
      });
