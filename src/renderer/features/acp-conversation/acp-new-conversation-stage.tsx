...
export function AcpNewConversationStage({
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      const result = await runtime.startThread({ sessionId, instanceConfigId, prompt });
      await onStarted(result.thread.threadId);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : `Could not start ${agentLabel}`);
      setPending(false);
    }
  };
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-token-main-surface-primary text-foreground">
      ...
      <div className="shrink-0 px-3 pb-3 pt-1">
        ...
        {error ? (
          <div className="mx-auto mt-1.5 w-full max-w-3xl px-1 text-xs text-danger">{error}</div>
        ) : null}
      </div>
    </div>
  );
