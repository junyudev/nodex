...
vi.mock("@/lib/renderer-transport", () => ({
  resolveRendererTransport: () => ({ subscribeComposerFileSearchEvents: mocks.subscribe }),
}));
...
test("coalesces edits during session creation, delivers only current results, and stops once", async () => {
  let emit!: (event: ComposerFileSearchEvent) => void;
  ...
  const receive = vi.fn();
  const session = createComposerFileSearchSession({ roots: ["/repo"], onEvent: receive });
  ...
  expect(mocks.control).toHaveBeenLastCalledWith("codex:composer-file-search:update", {
    sessionId,
    query: "abc",
  });
  ...
  expect(mocks.control).toHaveBeenLastCalledWith("codex:composer-file-search:stop", { sessionId });
});
...
test("closing while start is in flight prevents an update and still releases the native session", async () => {
  ...
  mocks.control.mockResolvedValue(undefined);
  const session = createComposerFileSearchSession({ roots: ["/repo"], onEvent: vi.fn() });
