...
export interface SidebarProjectManagerDataProps {
  projects: Project[];
  spaces: SpaceRef[];
...
interface LeftSidebarProjectManagerProps extends SidebarProjectManagerDataProps {
  onOpenSettings: () => void;
  projectPickerOpenTick: number;
}

interface ProjectManagerPopoverProps extends SidebarProjectManagerDataProps {
  open: boolean;
...
function getProjectManagerInputClassName() {
  return cn(
...
function ProjectManagerPopoverContent({
  projects,
  spaces,
  activeProjectId,
  onSelectSpace,
  ...
  onRenameProject,
  onRequestClose,
}: SidebarProjectManagerDataProps & { onRequestClose: () => void }) {
  ...
  const inputClassName = getProjectManagerInputClassName();
