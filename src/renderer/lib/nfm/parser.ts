...
export function parseNfm(input: string): NfmBlock[] {
  ...
  return normalizeChildlessBlocks(rootBlocks);
}
...
function parseCardToggle(
  ...
  while (i < lines.length) {
    const line = lines[i];
    const lineContent = line.slice(baseIndent);
    if (lineContent.trimEnd() === "</card-toggle>") {
      i++;
      break;
    }
  ...
  const hasExplicitTitleLine = firstLine.length === 0 || !looksLikeNfmBlockStart(firstLine);
  const titleLine = hasExplicitTitleLine ? firstLine : "";
  const childrenLines = hasExplicitTitleLine
    ? contentLines.slice(1)
    : contentLines;
  const children = parseNfm(childrenLines.join("\n"));
  ...
  return {
    block: {
      ...
      content: parseInlineContent(titleLine),
      children,
    },
    nextLine: i,
  };
...
function looksLikeNfmBlockStart(line: string): boolean {
  const trimmed = line.trimStart();
  if (trimmed.length === 0) return false;

  if (trimmed.startsWith("```")) return true;
  if (trimmed.startsWith("<card-ref")) return true;
  if (trimmed.startsWith("<card-toggle")) return true;
  if (trimmed.startsWith("<toggle-list-inline-view")) return true;
  if (trimmed.startsWith("<empty-block/>")) return true;
...
function normalizeChildlessBlocks(blocks: NfmBlock[]): NfmBlock[] {
  const normalized: NfmBlock[] = [];

  for (const block of blocks) {
    const normalizedChildren = normalizeChildlessBlocks(block.children);

    if (isChildlessNfmBlockType(block.type)) {
      normalized.push({
        ...block,
        children: [],
      });
