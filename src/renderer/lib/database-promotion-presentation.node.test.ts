...
const fixture = () => {
  const owner = new DatabasePromotionPresentationStore("identity", "window");
...
describe("Database promotion presentation", () => {
  test("ACK waits for the exact bounded read and the current rendered token", async () => {
    ...
    expect(
      owner.project({ ...read([]), windowKey: "other-window" }, new Set()).renderToken,
    ).toBeNull();
    const candidate = owner.project(read([]), new Set());
    ...
    expect(owner.getActivity().acknowledged).toBe(1);
    owner.markRendered(owner.project(read([]), new Set()).renderToken!);
